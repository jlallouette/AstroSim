...
string ExtraCellKStim::ClassName("ExtraCellularPotassiumStimulation");
...
void DefaultStimStrat::stimulateSpecific(StimulableCellNetwork *model, unsigned int ind) const
{
	...
	else if ((fdmod = dynamic_cast<FireDiffuseModel*>(model)))
	{
		double diffIP3 = (fdmod->GetDynVal(ind, FireDiffuseCell::C) < IP3Bias) ? 
			(fdmod->GetDynVal(ind, FireDiffuseCell::C) - IP3Bias) : 0.0 ;
		fdmod->ModifFluxes(ind, (*funct)(diffIP3));
	}
	else
...
//********************************************************************//
//******* E X T R A   C E L L   P O T A S S I U M   S T I M **********//
//********************************************************************//

//**********************************************************************
//Default Constructor from paramHandler
//**********************************************************************
ExtraCellKStim::ExtraCellKStim(ParamHandler & h) : 
	DefaultStimStrat(h)
{
	K = h.getParam<double>("-XtraCellKStim", 0);
	incrRate = h.getParam<double>("-XtraCellKStim", 1);
}

//**********************************************************************
// Default Constructor
//**********************************************************************
ExtraCellKStim::ExtraCellKStim(double _K, CouplingFunction *_f) :
	DefaultStimStrat::DefaultStimStrat(0, _f), K(_K)
{
}
...
// Constructor from stream
//**********************************************************************
ExtraCellKStim::ExtraCellKStim(std::ifstream & stream) : 
	DefaultStimStrat::DefaultStimStrat()
{
	LoadFromStream(stream);
}
...
// Returns a param handler of the stimulation strat parameters
//**********************************************************************
ParamHandler ExtraCellKStim::BuildModelParamHandler() 
{
	ParamHandler params;
	params += DefaultStimStrat::BuildModelParamHandler();

	params <= "ExtraCellKStim", K;
...
// Load the object from a stream
//**********************************************************************
bool ExtraCellKStim::LoadFromStream(std::ifstream & stream)
{
	bool ok = DefaultStimStrat::LoadFromStream(stream);
...
// Saves the object to a stream
//**********************************************************************
bool ExtraCellKStim::SaveToStream(std::ofstream & stream) const
{
	bool ok = DefaultStimStrat::SaveToStream(stream);
...
// Actually stimulate with appropriate method
//**********************************************************************
void ExtraCellKStim::stimulateSpecific(StimulableCellNetwork *model, unsigned int ind) const
{
	KChIModel *tmp = dynamic_cast<KChIModel*>(model);
	tmp->ModifKoutFluxes(ind, incrRate * (K - tmp->GetDynVal(ind, KChICell::Ko)));
}
